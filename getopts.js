...
var standardOpts = {
	...
	'outdir': {
		description: 'Where to dump output? (default: ./<wiki>/)',
		'boolean': false,
		'default': null,
	},
	'parsoidServer': {
...
function getopts(customOpts, defaults) {
	...
	var allOpts = extendProps(customOpts || {}, standardOpts);

	var usageStr = 'Usage: node ' + process.argv[1] + ' [options]';
	var opts = yargs.usage(usageStr, allOpts).check(checkUnknownArgs.bind(null, allOpts));
	var argv = opts.argv;
	...
	argv.prefix = argv.fileprefix || argv.title;
	argv.outdir = (argv.outdir || "./" + argv.wiki + "/").replace(/\/$/, '') + "/";
	argv.phpServer = wikiBaseUrl[argv.wiki];
