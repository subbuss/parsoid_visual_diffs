		...
		ph.createPage(function (page) {
			...
			page.set('onInitialized', function(success) {
				page.evaluate(function() {
					// Since we are replacing content in the DOM, wait till
					// the DOM is initialized. Without this, we get JS errors
					// when the PHP JS tries to access navigation on the DOM
					// which we are getting rid of in the callback below
				...
				page.includeJs("http://ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js", function() {
					page.evaluate(function(dumpOutput) {
						...
						if (content.nodeName === 'LINK' && content.getAttribute("rel") === "mw:PageProp/redirect") {
							...
							$('<style type="text/css">body.mw-body { padding: 1.25em 1.5em 1.5em 1.5em; }</style>').appendTo('head');
