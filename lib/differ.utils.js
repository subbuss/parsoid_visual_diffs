...
Util.computeOpts = function(opts) {
	...
	// Settings for generating HTML 1 (needs to be abstracted some more)
	var html1 = {};
	html1.name = opts.name1;
	html1.server = opts.html1Server || wikiBaseUrl[opts.wiki]; // Default to wikipedia server
	html1.server = html1.server.replace(/\/$/, '') + '/';
	html1.screenShot = filePath + '.' + opts.name1 + '.png';
	html1.url = html1.server + encodeURIComponent(opts.title);
	html1.baseHref = html1.server + opts.title;
	html1.postprocessorScript = opts.html1PP;
	html1.dumpHTML = opts.dumpHTML1;
	html1.dumpCSS = opts.dumpCSS1;
	html1.injectJQuery = false; // FIXME: hardcoded
	opts.html1 = html1;

	// Settings for generating HTML (needs to be abstracted some more)
	var html2 = {};
	html2.name = opts.name2;
	html2.server = opts.html2Server.replace(/\/$/, '') + "/";
	html2.screenShot = filePath + '.' + opts.name2 + '.png';
	html2.url = html2.server + opts.wiki + '/' + encodeURIComponent(opts.title);
	html2.baseHref = html2.server + opts.wiki + '/' + opts.title;
	html2.postprocessorScript = opts.html2PP;
	html2.dumpHTML = opts.dumpHTML2;
	html2.dumpCSS = opts.dumpCSS2;
	html2.injectJQuery = true; // FIXME: hardcoded
	opts.html2 = html2;

	opts.diffFile = filePath + ".diff.png";

	// Set up resemblejs outputSettings options
	if (!opts.outputSettings) {
		opts.outputSettings = {};
	}
	opts.outputSettings.largeImageThreshold = opts.largeImageThreshold;
	opts.outputSettings.errorType = opts.errorType;
	opts.largeImageThreshold = undefined;
